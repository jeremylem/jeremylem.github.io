...
- [ONNX](https://jeremylem.github.io/blogging/2025/11/30/AI_Java.html) runtime eliminates Python dependencies entirely handling the model execution natively
...
- LangChain4j mitigate the risk of silent tokenization failures: LangChain4j and the underlying
  [DJL](https://github.com/deepjavalibrary/djl)/[ONNX](https://jeremylem.github.io/blogging/2025/11/30/AI_Java.html) dependencies favor explicit, compiled code with fixed
...
* Embedding Model Specifications:\* the [All-MiniLM-L6-v2](https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2) model than the python implementation was choosen.
  Exactly for the same reasons at the time: high efficiency, producing a dense vector of **384 dimensions**. This dimension is automatically respected by LangChain4j when interfacing
  with **ChromaDB**.

- Chunking Strategy: the `RecursiveDocumentChunker` uses the `DocumentSplitters.recursive()` method, configured for **character count** (512 characters). This strategy
  intentionally keeps chunks safely below the model's hard limit of **256 tokens** (since 512 characters is roughly equivalent to 128-150 tokens in English), preventing truncation
  and maximizing context preservation.

The full source is available on GitHub for anyone interested in exploring RAG beyond the Python ecosystem: [mcpp_server4j_github](https://github.com/jeremylem/mcp_server4j)
